...
def initialize_anthropic_client():
    """Initialize the Anthropic client with API key from .env or environment"""
    global _anthropic_client
    if _anthropic_client is None:
        api_key = os.getenv("ANTHROPIC_API_KEY")
        if not api_key:
            raise ValueError("Anthropic API key not found. Please set ANTHROPIC_API_KEY in a .env file or as an environment variable.")
...
def translate_text_with_retry(text, source_lang, target_lang, client, max_retries=5):
    """Translate text using Claude 3.5 Sonnet with retry logic"""
        ...
        try:
            response = client.messages.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a professional translator. Provide only the translation inside square brackets."
                    },
                    {
                        "role": "user",
                        "content": prompt
...
def process_dataframe(df, source_lang, target_lang):
    ...
    print(f"Forward translation: Claude 3.5 Sonnet | Backtranslation: Google Translate")
    ...
    # Forward translations with Claude (rate limited)
    translations = []
    for i, text in enumerate(result_df['text']):
        ...
        if i < len(result_df) - 1:
            time.sleep(120)  # 1 request every 2 minutes for Claude
