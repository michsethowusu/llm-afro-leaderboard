...
def initialize_mistral_client():
    """Initialize the Mistral client with API key from .env or environment"""
    global _mistral_client
    if _mistral_client is None:
        api_key = os.getenv("MISTRAL_API_KEY")
        if not api_key:
            raise ValueError("Mistral API key not found. Please set MISTRAL_API_KEY in a .env file or as an environment variable.")
...
def translate_text_with_retry(text, source_lang, target_lang, client, max_retries=5):
    """Translate text using Mistral API with retry logic and square bracket delimiters"""
...
def process_dataframe(df, source_lang, target_lang):
    ...
    print(f"Forward translation: Mistral API | Backtranslation: Google Translate")
    ...
    # Forward translations with Mistral (rate limited)
    translations = []
    for i, text in enumerate(result_df['text']):
        ...
        if i < len(result_df) - 1:
            time.sleep(120)  # 1 request every 2 minutes for Mistral
