...
def create_enhanced_quadrant_chart(metrics_df, x_metric, y_metric, title, filename, outdir, 
    ...
    """
    Create an enhanced quadrant chart with smart label positioning to avoid overlaps
    """
    if metrics_df.empty:
        return None
    ...
    # Determine point sizes
    if size_metric and size_metric in metrics_df.columns:
        size_values = metrics_df[size_metric]
        ...
        if size_values.max() > size_values.min():
            ...
            size_normalized = pd.Series([0.5] * len(size_values))
        sizes = min_size + size_normalized * (max_size - min_size)
    else:
        sizes = pd.Series([25] * len(metrics_df))
    ...
    if color_metric and color_metric in metrics_df.columns:
        ...
        colors = px.colors.qualitative.Set3 * (len(metrics_df) // len(px.colors.qualitative.Set3) + 1)
        colors = colors[:len(metrics_df)]
        color_scale = None
        show_colorbar = False
    ...
    # Add quadrant background regions with better labels
    regions = [
        {
            'x0': metrics_df[x_metric].min() * 0.95, 'x1': x_line,
            'y0': y_line, 'y1': metrics_df[y_metric].max() * 1.05,
            'color': 'lightblue', 'label': 'Consistent\nBut Average',
            'label_x': x_line * 0.5, 'label_y': y_line + (metrics_df[y_metric].max() - y_line) * 0.5
        },
        {
            'x0': x_line, 'x1': metrics_df[x_metric].max() * 1.05,
            'y0': y_line, 'y1': metrics_df[y_metric].max() * 1.05,
            'color': 'lightgreen', 'label': 'LEADERS\n(High Performance & Consistency)',
            'label_x': x_line + (metrics_df[x_metric].max() - x_line) * 0.5,
            'label_y': y_line + (metrics_df[y_metric].max() - y_line) * 0.5
        },
        {
            'x0': metrics_df[x_metric].min() * 0.95, 'x1': x_line,
            'y0': metrics_df[y_metric].min() * 0.95, 'y1': y_line,
            'color': 'lightcoral', 'label': 'NEEDS IMPROVEMENT\n(Low Performance & Consistency)',
            'label_x': x_line * 0.5, 'label_y': y_line * 0.5
        },
        {
            'x0': x_line, 'x1': metrics_df[x_metric].max() * 1.05,
            'y0': metrics_df[y_metric].min() * 0.95, 'y1': y_line,
            'color': 'lightyellow', 'label': 'High Performance\nBut Inconsistent',
            'label_x': x_line + (metrics_df[x_metric].max() - x_line) * 0.5, 'label_y': y_line * 0.5
        }
    ]
    ...
    for region in regions:
        ...
        fig.add_annotation(
            x=region['label_x'], y=region['label_y'],
            text=region['label'], showarrow=False,
    ...
    positions = []
    label_spacing = {
        'x_offset': (metrics_df[x_metric].max() - metrics_df[x_metric].min()) * 0.02,
        'y_offset': (metrics_df[y_metric].max() - metrics_df[y_metric].min()) * 0.02
    }
    
    # Add data points with smart positioning
    for i, row in metrics_df.iterrows():
        x_val = row[x_metric]
        y_val = row[y_metric]
        label_text = row[id_column]
        
        # Determine label position to minimize overlaps
        text_position = "middle right"
        text_x_offset = label_spacing['x_offset']
        text_y_offset = 0
        
        # Adjust position based on quadrant and existing labels
        if x_val > x_line:  # Right side
            if y_val > y_line:  # Top right
                text_position = "bottom left"
                text_x_offset = -label_spacing['x_offset']
                text_y_offset = -label_spacing['y_offset']
            else:  # Bottom right
                text_position = "top left"
                text_x_offset = -label_spacing['x_offset']
                text_y_offset = label_spacing['y_offset']
        else:  # Left side
            if y_val > y_line:  # Top left
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                ...
                line=dict(width=2, color="white"),
                opacity=0.8,
                showscale=show_colorbar and i == 0,  # Only show colorbar once
                colorbar=dict(title=color_metric.replace('_', ' ').title()) if show_colorbar and i == 0 else None
            ),
            showlegend=False,
            hovertemplate=(
                f"<b>{label_text}</b><br>" +
                f"{x_metric.replace('_', ' ').title()}: {x_val:.2f}<br>" +
                f"{y_metric.replace('_', ' ').title()}: {y_val:.2f}<br>" +
                (f"{size_metric.replace('_', ' ').title()}: {row[size_metric]}<br>" if size_metric else "") +
    ...
    # Save chart
    fig.write_html(os.path.join(outdir, f"{filename}.html"))
    fig.write_image(os.path.join(outdir, f"{filename}.png"), width=900, height=700)
    
    return fig
    """
    Create a quadrant chart showing model positioning
    """
    if metrics_df.empty:
        return None
    ...
    fig.add_annotation(
        x=x_line * 0.7, y=y_line + (metrics_df[y_metric].max() - y_line) * 0.5,
        text="High Performance<br>Low Efficiency", showarrow=False,
        font=dict(size=12, color="gray"), opacity=0.7
    )
    ...
    fig.add_annotation(
        x=x_line + (metrics_df[x_metric].max() - x_line) * 0.5,
        y=y_line + (metrics_df[y_metric].max() - y_line) * 0.5,
        text="LEADERS<br>(High Performance & Efficiency)", showarrow=False,
        font=dict(size=14, color="darkgreen", weight="bold"), opacity=0.8
    )
    ...
    fig.add_annotation(
        x=x_line * 0.7, y=y_line * 0.7,
        text="UNDERPERFORMERS<br>(Low Performance & Efficiency)", showarrow=False,
        font=dict(size=12, color="darkred"), opacity=0.7
    )
    ...
    fig.add_annotation(
        x=x_line + (metrics_df[x_metric].max() - x_line) * 0.5, y=y_line * 0.7,
        text="High Efficiency<br>Low Performance", showarrow=False,
        font=dict(size=12, color="orange"), opacity=0.7
    )
    ...
    for i, row in metrics_df.iterrows():
        fig.add_trace(go.Scatter(
            ...
            textfont=dict(size=10, color="black"),
            marker=dict(
                size=sizes[i] if hasattr(sizes, '__getitem__') else sizes,
                color=colors[i] if isinstance(colors, list) else colors.iloc[i] if hasattr(colors, 'iloc') else colors,
                colorscale=color_scale if color_scale else None,
                line=dict(width=2, color="white"),
                opacity=0.8
            ),
